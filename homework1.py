...
def split_set ( num, ratio, data_set ):
    ...
    for _ in range(num):
        ...
        for j in range(len(splited_x_temp)):
            row = []
            col = []
            data = []
            for i in range(len(splited_x_temp[j])):
                for ele in splited_x_temp[j][i]:
                    row.append (i)
                    col.append (int(ele[0]) - 1)
                    data.append ((float(ele[1])))
            sparse = csr_matrix ( (data, (row, col)), shape = (len(splited_y[j]), argnum), dtype =
                    float)
            splited_x.append(np.mat(sparse.toarray()))
...
def cost_function_linear( y, x, beta ):
    ...
    temp_square = (beta.dot(x.transpose())-y).dot(x)
...
def prediction_error_linear ( y, x, beta ):
    ...
    return np.sum(np.abs(y - beta.dot(x.transpose())))
...
def prediction_error_logistic ( y, x, beta ):
    ...
    return np.sum(np.abs(y - sigmoid(beta.dot(x.transpose()))))

def backtracking_line_search ( y, x, beta, step_size, back):
    # print "Calculate gradient descent ..."
...
if __name__ == "__main__":
    ...
    num_set = 10
    iteration = 100
    linear_gradient_step_size = 0.1
    logistic_gradient_step_size = 5
    ...
    for splited_data in splited_data_set:
        ...
        if first:
            cost_analytic = iteration * [cost_function_linear (test_set_y, test_set_x,
                beta_linear_analytic)]
            first = False
